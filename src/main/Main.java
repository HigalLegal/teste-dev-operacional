...
public class Main {
				...
				if ((usuarioLogado.getSenha().equals(senha))) {
						...
						case 1: {
							...
							vendas.forEach(venda -> {
								if (venda.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
									...
									System.out.println("Total Venda: R$" + venda.totalValor());
									System.out.println("Total Taxa a ser paga: R$" + venda.comissaoDoSistema());
									System.out.println("Total Líquido  para empresa"
											+ (venda.totalValor().subtract(venda.comissaoDoSistema())));
									System.out.println("************************************************************");
								}
					...
					if (usuarioLogado.isCliente()) {
						...
						case 1: {
							...
							System.out.println("Total: R$" + venda.totalValor());
							System.out.println("************************************************************");
							carrinho.clear();
							executar(usuarios, clientes, empresas, produtos, carrinho, vendas);
						}
	...
	public static Venda criarVenda(List<Produto> carrinho, Empresa empresa, Cliente cliente, List<Venda> vendas) {
		...
		Venda venda = new Venda(idVenda, carrinho, empresa, cliente);
		empresa.setSaldo(empresa.getSaldo().add(venda.totalValor())); // +
		vendas.add(venda);
		return venda;
	}
}
